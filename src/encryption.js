...
import { CheckoutButtonTemplateEndpointException } from "./endpoint.js";
...
export const decryptRequest = (body, privatePem, passphrase) => {
  ...
  try {
    ...
    */
    throw new CheckoutButtonTemplateEndpointException(421, {
      error_msg: `Failed to decrypt the request. Please verify your private key.`,
    });
  }

  // decrypt checkout data
  const checkoutDataBuffer = Buffer.from(encrypted_flow_data, "base64");
  const initialVectorBuffer = Buffer.from(initial_vector, "base64");

  const TAG_LENGTH = 16;
  const encrypted_flow_data_body = checkoutDataBuffer.subarray(0, -TAG_LENGTH);
  const encrypted_flow_data_tag = checkoutDataBuffer.subarray(-TAG_LENGTH);
  ...
  console.log("encrypted_flow_data_tag", encrypted_flow_data_tag);
